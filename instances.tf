...
aws ec2 wait instance-status-ok --region ${var.region-master} --instance-ids ${self.id} \
 && ansible-playbook -i ansible_templates/inventory_aws/tf_aws_ec2.yml --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name}'
...
&& ansible-playbook -i ansible_templates/inventory_aws/tf_aws_ec2.yml --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name} master_ip=${aws_instance.jenkins-master.private_ip}'
EOF
  }
