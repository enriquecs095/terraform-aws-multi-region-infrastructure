...
aws ec2 wait instance-status-ok --region ${var.region-master} --instance-ids ${self.id} \
 && ansible-playbook -i ${self.public_ip} --private-key e_key  --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name}' ansible_templates/jenkins-master-sample.yml
EOF
  }
  tags = {
...
&& ansible-playbook -i ${self.public_ip} --private-key e_key --extra-vars 'passed_in_hosts=tag_Name_${self.tags.Name} master_ip=${aws_instance.jenkins-master.private_ip}' ansible_templates/jenkins-worker-sample.yml
EOF
  }
