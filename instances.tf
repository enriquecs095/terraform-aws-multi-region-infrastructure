...
aws ec2 wait instance-status-ok --region ${var.region-master} --instance-ids ${self.id} \
 && ansible-playbook -l ${self.public_ip} --private-key e_key  ansible_templates/jenkins-master-sample.yml
EOF
  }
  tags = {
...
&& ansible-playbook -l ${self.public_ip} --private-key e_key  ansible_templates/jenkins-worker-sample.yml
EOF
  }
